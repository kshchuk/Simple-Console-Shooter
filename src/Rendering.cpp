	...
	void Collision(Player& player, float fElapsedTime, char key, const std::vector<std::vector<bool>>& map)
	{
		if (key == 'W')
		{
			if (sinf(player.rotation) > 0 && cosf(player.rotation) > 0) {
				...
				if (map[static_cast<int> (player.x)][static_cast<int> (player.y)]) { 
					player.x -= sinf(player.rotation) * player.speed * fElapsedTime;
		...
		for (int x = 0; x < conf.screenWidth; x++)
		{
			...
			while (!bHitWall && !bHitPlayer && fDistanceToObstacle < player.depth)
			{
				...
				if (fTestX < 0 || fTestX >= conf.mapWidth || fTestY < 0 || fTestY >= conf.mapHeight)
				{
					...
					if (map[static_cast<int> (fTestX)][static_cast<int> (fTestY)])
					{
						// Ray has hit wall
						bHitWall = true;
						...
						for (int tx = 0; tx < 2; tx++)
							for (int ty = 0; ty < 2; ty++)
							{
								...
								float vy = static_cast<int> (fTestY + ty - player.y);
								float vx = static_cast<int> (fTestX + tx - player.x);
					...
					else // Ray is inbounds so test to see if the ray cell is a player
					{
						if (other_players)
							for (auto iter : *other_players)
							{
								...
								if (rendering::distance(cur_player->x, cur_player->y, player.x, player.y) > conf.depth)	// if player isn't in the reach zone
									continue;
								else
								{
									...
									if (rendering::cross(player.x, player.y, fTestX, fTestY,
										...
										rendering::cross(player.x, player.y, fTestX, fTestY,
											cur_player->leftFront_pos.x, cur_player->leftFront_pos.y,
